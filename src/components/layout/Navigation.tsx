...
//       <nav 
//         ref={navRef}
//         className={`hidden md:block fixed top-0 left-0 w-full z-50 transition-all duration-500 nav-container
//           ${isScrolled 
...
//               className={`flex px-6 py-2 border rounded-full 
...
//       {/* Mobile: Juste l'icône hamburger flottante */}
//       <button 
...
//       {/* Menu Mobile - Contenu plein écran */}
//       {isMobileMenuOpen && (
//         <div className="md:hidden fixed top-0 left-0 w-full h-full z-40 bg-black/95 backdrop-blur-lg">
//           <div className="px-4 pt-20 pb-4 space-y-2">
//             {menuItems.map(({ id, label }) => (
//               <button
//                 key={`mobile-${id}`}
//                 onClick={() => handleNavClick(id)}
//                 className={`block w-full py-4 px-4 rounded-md text-center text-xl
...
//             <button
//               onClick={() => handleNavClick('contact')}
//               className={`block w-full mt-6 py-4 px-4 rounded-md text-center text-xl
//                 transition-all duration-300 ${
//                   activeSection === 'contact'
...
// 🚀 components/layout/Navigation.tsx - VERSION CORRIGÉE
import { useState, useEffect, useCallback, useRef, memo } from 'react';
import { gsap } from 'gsap';
import { useNavigation } from '../../hooks/useNavigation';
import { useActiveSection } from '../../hooks/useActiveSection';
...
const Navigation = () => {
  ...
  // 🔥 CORRECTION : Animation d'entrée avec vérification du DOM
  useEffect(() => {
    ...
    }, 100); // Petit délai pour s'assurer que le DOM est monté
            ...
            <div className="flex items-center space-x-8">
              {menuItems.map(({ id, label }) => (
                <button
                  key={id}
                  onClick={() => handleNavClick(id)}
                  className={`nav-link relative px-2 py-2 text-lg transition-all duration-300 group ${
                    activeSection === id 
                      ? 'text-[#4ADE80] font-semibold' 
                      : 'text-white hover:text-[#4ADE80]'
                  }`}
